...
def main(args):
    ...
    random_scalar = torch.tensor([0.5]).to(args.device)
    for key, param in denoise_adj_ood.state_dict().items():
        noise = torch.ones_like(param).to(args.device) * random_scalar  
        param.add_(noise)
    for key, param in denoise_x_ood.state_dict().items():
        noise = torch.ones_like(param).to(args.device) * random_scalar  
        param.add_(noise)
    generator_ood["model_adj"] = denoise_adj_ood
    generator_ood["model_x"]= denoise_x_ood
