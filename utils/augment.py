...
def build_augmentation_dataset(args, model, generator, labeled_data):
    ...
    for step, batch_data in enumerate(labeled_trainloader):
        ...
        if batch_data.x.shape[0] > 1:
            ...
            random_indices = torch.randint(low=0,high=batch_data.y.shape[0],size=[1,],dtype=torch.float32)
            augment_mask = torch.zeros(y_pred_logits.size(0)).to(y_pred_logits.device).scatter_(0, topk_indices, 1).bool()
            random_augment_mask = torch.zeros(y_pred_logits.size(0)).to(y_pred_logits.device).scatter_(0, random_indices, 1).bool()
            ...
            def get_aug_grads(prediction_model, inner_output_data):
                ...
                with torch.enable_grad():
                    ...
                    if inner_output_x.shape[0] > 1:
                        ...
                        query_structure_embed, pos_structure_embed, neg_structure_embed = aug_feats, ori_feats[augment_mask], ori_feats[neg_indices] #B*F,B*F,B*N_nag*F->
                        #每个batch选topk个图作为生成模板，一次生成topk个图，其中每个图对应自己的原始输入为正样本，每个图再选neg_indices个负样本，计算INFONCE
                        #our methodd 每个batch随机选一个图作为生成模板，一次生成一个图，这个图对应batch其他图为正样本，扰动diffusion 参数再生成一个负样本，计算infornce，可以先warmup一下，前几个生成先不加负样本
