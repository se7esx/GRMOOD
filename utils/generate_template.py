...
from .mol_utils import convert_dense_to_rawpyg, convert_sparse_to_dense, combine_graph_inputs, convert_dense_adj_to_sparse_with_attr,convert_to_batch
...
def build_augmentation_dataset(args, generator, generator_ood,labeled_data):
    ...
    for step, batch_data in enumerate(labeled_trainloader):
        ...
        if batch_data.x.shape[0] >= 1:
            ...
            random_indices = torch.randint(low=0,high=batch_data.y.shape[0],size=[1,],dtype=torch.int64,device=args.device)
            random_indices_batch = torch.randint(low=0,high=batch_data.y.shape[0],size=[args.topk,],dtype=torch.int64,device=args.device)
            random_augment_mask = torch.zeros(batch_data.y.shape[0]).to(batch_data.y.device).scatter_(0, random_indices, 1).bool()

            random_batch_mask = torch.zeros(batch_data.y.shape[0]).to(batch_data.y.device).scatter_(0, random_indices_batch, 1).bool()
            ...
            ori_x, ori_adj = ori_x.to(torch.float64), ori_adj.to(torch.float64)
            ...
            batch_dense_x_select, batch_dense_enable_adj_select = batch_dense_x[random_augment_mask], batch_dense_enable_adj[random_augment_mask]
            if batch_dense_x_disable is not None:
                batch_dense_x_disable_select, batch_dense_disable_adj_select = batch_dense_x_disable[random_augment_mask], batch_dense_disable_adj[random_augment_mask]
            ...
            z_x = gen_noise(batch_dense_x_select.to(torch.float32), flags=batch_node_mask[random_augment_mask], sym=False, perturb_ratio=args.perturb_ratio)
            ...
            z_adj = gen_noise(batch_dense_enable_adj_select.to(torch.float32), flags=batch_node_mask[random_augment_mask], sym=True, perturb_ratio=args.perturb_ratio)
                ...
                with torch.enable_grad():
                    ...
                    if inner_output_x.shape[0] >= 1:
                        ...
                        list_h = [th.ones(C.shape[0], dtype=th.float64, device='cpu') / C.shape[0] for C in
                                  list_of_tensors]
                        hbar = [th.ones(inner_output_adj.shape[1], dtype=th.float64, device='cpu') / inner_output_adj.shape[1]]
                        adj_tmp = [inner_output_adj.squeeze(dim=0).requires_grad_().to('cpu')]
                        fea_tmp = [inner_output_x.squeeze(dim=0).requires_grad_().to('cpu')]
                        print("--------------loss_id------------------")
                        loss_id = parallelized_get_features(1,list_of_tensors,list_of_features,list_h,adj_tmp,fea_tmp,hbar,th.tensor(0.5,dtype = torch.float64)).mean()
                        print("--------------loss_ood------------------")
                        loss_ood = parallelized_get_features(1,[ood_adj.squeeze(dim=0).to('cpu')],[ood_x.squeeze(dim=0).to('cpu')],[th.ones(ood_adj.shape[1], dtype=th.float64, device='cpu') / ood_adj.shape[1]],adj_tmp,fea_tmp,hbar,th.tensor(0.5,dtype = torch.float64)).mean()
            ...
            for key, param in denoise_adj_ood.state_dict().items():
                noise = torch.ones_like(
                    param) * random_scalar  # 或者使用 torch.rand_like(param) * scale - offset 生成具体范围的随机扰动
                param.add_(noise)
            for key, param in denoise_x_ood.state_dict().items():
                noise = torch.ones_like(param) * random_scalar  # 或者使用 torch.rand_like(param) * scale - offset 生成具体范围的随机扰动
                param.add_(noise)
            
            inner_output_ood_x, inner_output_ood_adj = inner_sampling(args, generator_ood, perturbed_x, perturbed_adj, sde_x, sde_adj, args.out_steps, batch_node_mask[random_augment_mask])# ood 样本
            ...
            for i in outer_iters:
                inner_output_x, inner_output_adj = inner_sampling(args, generator, perturbed_x, perturbed_adj, sde_x, sde_adj, args.out_steps-i, batch_node_mask[random_augment_mask])
                aug_grad_x, aug_grad_adj = get_aug_grads([inner_output_x, inner_output_adj],[batch_dense_x[random_batch_mask], batch_dense_enable_adj[random_batch_mask]],[inner_output_ood_x, inner_output_ood_adj ])
                with torch.no_grad():
                    t = timesteps[i]
                    vec_t = torch.ones(perturbed_adj.shape[0], device=t.device) * t
                    _x = perturbed_x
                    perturbed_x, perturbed_x_mean = corrector_obj_x.update_fn(perturbed_x, perturbed_adj, batch_node_mask[random_augment_mask], vec_t, aug_grad=aug_grad_x)
                    perturbed_adj, perturbed_adj_mean = corrector_obj_adj.update_fn(_x, perturbed_adj, batch_node_mask[random_augment_mask], vec_t, aug_grad=aug_grad_adj)
                    _x = perturbed_x
                    perturbed_x, perturbed_x_mean = predictor_obj_x.update_fn(perturbed_x, perturbed_adj, batch_node_mask[random_augment_mask], vec_t, aug_grad=aug_grad_x)
                    perturbed_adj, perturbed_adj_mean = predictor_obj_adj.update_fn(_x, perturbed_adj, batch_node_mask[random_augment_mask], vec_t, aug_grad=aug_grad_adj)
