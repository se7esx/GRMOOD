...
def convert_sparse_to_dense(batch_index, node_feature, edge_index, edge_attr, augment_mask=None, return_node_mask=True):
    ...
    #print(node_feature[:,0].view(-1))
    #print(node_feature[:,0].view(-1).shape)
    atomic_num = node_feature[:,0].view(-1) + 1
    #print(atomic_num)
    #print(atomic_num.shape)
    ...
    #print(dense_x_disable[:,:,:10])
    #print(dense_x_disable.shape)
    if return_node_mask:
...
def combine_graph_inputs(x, x_disable, adj, adj_disable, mode='continuous'):
    ...
    if mode == 'discrete':
        adj = quantize_mol(adj).to(adj_disable.device)
    else:
        adj = standardize_adj(adj).to(adj_disable.device)
...
def bond_to_feature_vector(bond):
    ...
    return bond_feature
def convert_to_batch(batch_index, node_feature, edge_index, edge_attr, augment_mask=None, return_node_mask=True):
    edge_attr = edge_attr[:,0].view(-1) + 1 # index 0/1/2/3/4 to bond type S, D, T, AROMATIC and misc
    max_count = torch.unique(batch_index, return_counts=True)[1].max()
    # process edge matrix B, N, N
    enable_edge_attr, disable_edge_attr = split_edge_attr(edge_attr)
    ...
    dense_atomic_num, node_mask = to_dense_batch(node_feature, batch=batch_index, max_num_nodes=max_count) # B, N=max_cout, F=None
    dense_x_disable = torch.zeros([dense_atomic_num.shape[0],dense_atomic_num.shape[1],115])
    if return_node_mask:
        return dense_atomic_num, dense_x_disable, dense_enable_adj, dense_disable_adj, node_mask
    else:
        return dense_atomic_num, dense_x_disable, dense_enable_adj, dense_disable_adj